...
//        baseView.setTargetToPanGestureRec(target: self, action: #selector(panCard))
//    }
...
/*
    @objc func panCard(_ sender: UIPanGestureRecognizer) {
      ...
      if absXFromCenter > 100 {
        ...
        baseView.thumbImageView.alpha = absXFromCenter / view.center.x
        baseView.thumbImageView.tintColor = isRight ? .green : .red
        baseView.thumbImageView.image = UIImage(systemName: isRight ? "hand.thumbsup.fill" : "hand.thumbsdown.fill")
...
//              self.titleLabel.text = self.game.getWord(result: .yes)
    ...
    func resetCard() {
      UIView.animate(withDuration: 0.2) {
          self.baseView.cardView.center = self.view.center
          self.baseView.thumbImageView.alpha = 0
          self.baseView.cardView.transform = .identity
      }
    }
 */
